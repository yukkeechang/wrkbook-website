...
Meteor.methods({
  ...
  checkIfProandProWork(jobId, revieweeId, userId) {
    ...
    let currentUser = Meteor.users.findOne(
      { _id: userId },
      { fields: { "profile.employeeData.prevJobs": 1 } }
    );
    let toBeReviewed = Meteor.users.findOne(
      { _id: revieweeId },
      { fields: { "profile.employeeData.prevJobs": 1 } }
    );
    if (currentUser.profile.employeeData.prevJobs.length === 0) return false;

    let currentUserWorked = currentUser.profile.employeeData.prevJobs.indexOf(
      jobId
    );
    let toBeReviewedWorked = toBeReviewed.profile.employeeData.prevJobs.indexOf(
      jobId
    );
  ...
  validateReview(reviewObject) {
    ...
    let reviewerId = !validations.validate(reviewObject, {
      keys: ["reviewerId"]
    });
    let revieweeId = !validations.validate(reviewObject, {
      keys: ["revieweeId"]
    });
    ...
    if (nukeText) {
      ...
      if (revieweeId || reviewerId || jobId || rating || proReview || conReview)
  ...
  */
  createReview(newReview) {
    ...
    let jobInfo = Job.findOne(
      { _id: newReview.jobId },
      { fields: { employerId: 1 } }
    );
    let employerInfo = Meteor.users.findOne({ _id: jobInfo.employerId });
    newReview.companyName.text =
      employerInfo.profile.employerData.companyName.text;

    Meteor.call("validateReview", newReview);
    let reviewExist = Meteor.call(
      "checkIfReviewForJobExist",
      newReview.jobId,
    ...
    if (isCON && Roles.userIsInRole(newReview.revieweeId, PROFESSIONAL)) {
      let workedOnJobs = Meteor.call(
        "checkIfConandProWork",
        newReview.jobId,
        newReview.revieweeId,
        this.userId
      );
  ...
  */
  updateReview(reviewId, newReview) {
    ...
    if (newReview.review != DEFAULT) {
      // Check if the text provided is new user text
    ...
    Review.update(
      { _id: reviewId, reviewerId: this.userId },
      { $set: prevReview }
    );
  },
  /**
